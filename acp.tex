...
\vspace{0.1in}
The paper is organized around three linked questions that any credible AI-for-democracy system must answer. First, \emph{why use it}: where does ACP add value over chronological review, participation platforms, opinion mapping, generic AI summaries, or human facilitation alone? Second, \emph{how to measure success}: which failure modes should count, and which outcomes should not be treated as legitimacy proxies? Third, \emph{how to generate meaningful evidence quickly}: what minimum technical infrastructure can produce matched baselines, traces, adjudication, and human-review packets before any field-efficacy claim is made? Our answer is that these cannot be separated. Adoption without measurement becomes marketing; measurement without a data-producing workflow becomes metrics theater; infrastructure without a democratic use case becomes product drift.
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item We tie the protocol to three practical evaluation questions---adoption fit, success measurement, and fast evidence generation---through a bounded evidence stack: a flagship public-hearing benchmark, a measured behavioral intervention study, conformance and release-discipline surfaces, and explicit non-claims that separate inspectable workflow evidence from operator utility or field efficacy.
\end{enumerate}
...
\vspace{0.1in}
This framing also clarifies the practical program ACP is trying to make possible. The adoption question is not ``why use another deliberation platform?'' but ``when should institutions demand a protocol-bounded attention layer for public input?'' The measurement question is not ``did the model produce a nice summary?'' but ``did the workflow reduce the failure modes that matter for democratic accountability?'' The technical MVP question is not ``can we build a full civic product?'' but ``can we generate meaningful, reviewable evidence fast through matched baselines, traces, adjudication, and human-review packets?'' These three questions are the spine of the project. If any one is pursued alone, ACP drifts either into marketing, metrics theater, or infrastructure without civic meaning.
