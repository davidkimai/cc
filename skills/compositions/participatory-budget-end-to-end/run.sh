...
out_dir="${1:-skills/compositions/participatory-budget-end-to-end/out/participatory-budget-end-to-end}"
mkdir -p "$out_dir"

npm run --silent benchmark:compare -- --class participatory-budget-priorities --out "$out_dir/comparison" >/dev/null
...
const payload = {
  ...
  claimBoundary: 'budget prioritization evidence is decision-support, not proof of allocation legitimacy or consensus',
};
